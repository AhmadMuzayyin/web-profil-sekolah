...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Edit Guru/Staff</h1>
</div>
...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <img src="{{ Storage::url($teacher->photo) }}" alt="{{ $teacher->name }}" class="w-32 h-32 object-cover rounded">
        </div>
        @endif
        ...
        <div class="mb-4">
            ...
            <textarea name="bio" id="bio" rows="4"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('bio') border-red-500 @enderror">{{ old('bio', $teacher->bio) }}</textarea>
            @error('bio')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $teacher->sort_order) }}"
